...
class YandexGPTPromptBuilder(AbstractPromptBuilder):
    ...
    def build_text_content_prompt(self, user_data: PromptContext, user_text: str) -> str:
        ...
        sections.extend([
            ...
            "• НЕ ВСТАВЛЯЙ одиночные звездочки '*' и подчеркивания '_' НИ В КОЕМ СЛУЧАЕ!!!"
        ])
    ...
    def build_card_content_prompt(self, user_data: PromptContext, full_content: str) -> str:
        ...
        sections = [
            "Задача: создай короткое предложение для информационной карточки на основе полного поста.",
        ]
        ...
        sections.extend([
            ...
            "• Без хештегов",
            "• Без переходов на новую строку, без звездочек *, без разделителей ---",
        ])
    ...
    def build_image_prompt_prompt(self, user_data: PromptContext, generated_post: str) -> str:
        sections = [
            "Задача: промпт для генерации изображения к посту организации НКО в социальной сети.",
        ...
        sections.append(
            f"Вот пост, для которого нужно сделать промпт изображения: {generated_post}"
        )

        sections.append("Примеры промптов под контент поста:")
        sections.extend(
            [
                '• (Пост про защиту животных) Промпт: "Человек сидит на лавочке с котом."',
                '• (Пост про поддержку пожилых людей) Промпт: "Пожилой человек идет по лесу."',
            ]
        )
        sections.append(
            "Если на изображении фигурируют люди, обязательно добавь данный текст: "
            "'У людей нормальные руки и ноги, люди изображены отчетливо.'"
        )
